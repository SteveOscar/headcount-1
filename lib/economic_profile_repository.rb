...
class EconomicProfileRepository
  ...
  def check_for_existing_objects(data_type)
    if economic_profile.nil?
      ...
      create_economic_profile_objects(data_type, parser.get_enrollment) unless data_type.to_s.include?("lunch")
    else
      append_economic_profile(data_type, test_parser.get_testing_data) if data_type.to_s.include?("lunch")
      append_economic_profile(data_type, parser.get_enrollment) unless data_type.to_s.include?("lunch")
    end
  end
